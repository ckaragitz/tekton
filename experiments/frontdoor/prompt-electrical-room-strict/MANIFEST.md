...
**Tool:** tekton frontdoor (rvt.frontdoor) v1.0.0 · 2026-08-04T17:50:10Z
...
- feeder edges: 7
- intent JSON: `experiments/frontdoor/prompt-electrical-room-strict/intent.json`
...
## Build
- degrade mode: **split-strict** — --strict: the walls+loaded-families combination is the OPEN BUG, so the room is emitted as TWO coordinated files -- 'shell' (the 4 walls on the base) and 'equipment' (the 7 loaded families + their placed instances on the base) -- each a viewer-certified SHAPE (walls-only PASS; families+placement PASS).
...
- file **shell**: `experiments/frontdoor/prompt-electrical-room-strict/electrical_room_prompt-shell.rvt` (581632 bytes, sha256 `50965046d70ef99a…`)
- file **equipment**: `experiments/frontdoor/prompt-electrical-room-strict/electrical_room_prompt-equipment.rvt` (651264 bytes, sha256 `602481a3439c0ea5…`)
...
- self-checks **shell**: validator VALID (0 errors, 1 warnings); registries coherent=True; identity PASS
- self-checks **equipment**: validator VALID (0 errors, 1 warnings); registries coherent=True; identity PASS
- deliverability (P0 gate): PROOF-ONLY, NOT-DELIVERABLE — P0 genesis gate G1 fails (the base carries derived expression that G1 blocks): FAIL — 3,056 Autodesk-derived element(s) in expression-bearing categories. Nothing built on a sample base is a product; ship only from a certified genesis base.
- **degradation**: feeder CIRCUITS not authored: NO CIRCUIT SPECIMEN in the base or the loaded file (the family-free structural-lineage base carries no RbsElectricalSystem; rvt.mutate.add_circuit CLONES one) -- an RbsElectricalSystem CONSTRUCTOR is the exact missing piece.  The feeder tree is fully resolved (edges + ratings + poles + voltage, corroborated by the conduit geometry) and every placed board carries an unconnected 50000-series slot per outgoing feeder, so each edge is one add_circuit(panel, load) call once a constructor / same-commit circuit template exists.  (LookupError: electrical_room_prompt-equipment: no 2-connector circuit specimen to clone) -- the resolved circuit PLAN rides in the manifest (rvt.mep add_circuit / a Revit-side add-in build them from it)
...
- entrypoint: `python tools/frontdoor.py author --rvt experiments/frontdoor/prompt-electrical-room-strict/electrical_room_prompt-equipment.rvt --edit "<sentence | ops.json | inline JSON>" --out <dir>`
...
- `MSB` (id 1472942, equipment-instance): move: `move MSB to X,Y,Z ft`; retype: `retype MSB to <symbol id>`; delete: `delete MSB with cascade`
- `DP-1` (id 1472943, equipment-instance): move: `move DP-1 to X,Y,Z ft`; retype: `retype DP-1 to <symbol id>`; delete: `delete DP-1 with cascade`
- `DP-2` (id 1472944, equipment-instance): move: `move DP-2 to X,Y,Z ft`; retype: `retype DP-2 to <symbol id>`; delete: `delete DP-2 with cascade`
- `LP-1` (id 1472945, equipment-instance): move: `move LP-1 to X,Y,Z ft`; retype: `retype LP-1 to <symbol id>`; delete: `delete LP-1 with cascade`
- `LP-2` (id 1472946, equipment-instance): move: `move LP-2 to X,Y,Z ft`; retype: `retype LP-2 to <symbol id>`; delete: `delete LP-2 with cascade`
- `LP-3` (id 1472947, equipment-instance): move: `move LP-3 to X,Y,Z ft`; retype: `retype LP-3 to <symbol id>`; delete: `delete LP-3 with cascade`
- `LP-4` (id 1472948, equipment-instance): move: `move LP-4 to X,Y,Z ft`; retype: `retype LP-4 to <symbol id>`; delete: `delete LP-4 with cascade`
