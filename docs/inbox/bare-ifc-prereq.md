...
| File | Change |
|---|---|
| `plugin/skills/_shared/tekton_env.py` (hand-authored, stdlib-only) | `ROUTE_EXTRAS = {"ifc": ("numpy",)}` — the one table; `_route_capability(extras)` → `preflight()["routes"] = {prompt:{ok,needs}, ifc:{ok,needs[,fix]}, rvt:{ok,needs}}` from the `find_spec` probe preflight already did (nothing imported, +0 ms); READY line gains `ifc-route OK` / `ifc-route needs numpy (python -m pip install numpy)`; `_gated_route(script, args)` spots `frontdoor.py … --ifc FILE` / `--ifc=FILE`; `go()` short-circuits such a job when `routes.ifc.ok` is false through the same `not_ready()` exit the environment-NOT-READY path now uses (`go.ready:false`, `go.exit_code:3`, `go.preflight` detail, `result:null`) plus `go.route`, `go.prerequisite = {route, needs, fix}` and `go.preflight_line = "tekton: NOT READY for --ifc \| ifc-route needs numpy: python -m pip install numpy (one-time; the other routes are READY without it)"` — which the SKILL.md's existing rule (*`go.ready:false` → relay `preflight_line` verbatim, it names the one thing wrong*) already handles with **zero SKILL.md edits**; `doctor` prints its ifc hint from the same table; docstrings say so. |
| `src/rvt/frontdoor/__init__.py` `_route_ifc` (the boundary, 5 lines) | `except ImportError` ahead of the generic handler: `IFC intent failed: the --ifc route needs numpy, not installed on this interpreter -- one-time fix: python -m pip install numpy (--prompt / --rvt run without it)` — module name from `e.name` / `e.__cause__.name` (the `_lazyimp` proxy chains the real `ModuleNotFoundError`), exactly 160 chars so `status = FAILED (…)` carries it whole; keeps the `IFC intent failed:` prefix `router.py`'s nothing-buildable detection keys on. `src/rvt/ifc/intent.py` untouched (FENCED). |
...
| `plugin/lib/src/rvt/frontdoor/__init__.py` | byte-identical mirror written by `tools/sync_plugin.py`. |
...
- **A — numpy-free `--ifc` intent resolution** (`src/rvt/ifc/intent.py` placement-chain + footprint math on stdlib, or a steplite-style numpy-subset shim), DONE = `go author --ifc` under `python3 -I -S` builds the combined `.rvt`, VALID 0, `surface_bench author-ifc PASS` on cowork/codeexec with `numpy=NO`, and `test_ifc_route_prerequisite_table_matches_the_engine` flipped to assert success + `ROUTE_EXTRAS` emptied.  Blocked on the #498 fence lifting for `intent.py`.  → filed as a task issue `Refs #127`.
- **surface_bench: classify a preflight-stated prerequisite as BLOCKED, not FAIL** (`tools/surface_bench.py`, #113's territory).  Patch, for whoever holds the file:
  ...
  ```
  DONE = bench exits 0 on a numpy-less image with `author-ifc BLOCKED (…needs numpy…)`, still FAIL on any other ifc failure; `tests/test_surface_bench_reason.py` gains the case.  → filed as a task issue `Refs #127 #113`.
...
- Whole merged CI shard: see BRANCH STATE (filled after the run).

## BRANCH STATE

- **Branch:** `cam/127-bare-ifc-prereq` from `origin/main` @ fdcbf12.
- **Files written:** `plugin/skills/_shared/tekton_env.py`, `src/rvt/frontdoor/__init__.py` (+ mirror `plugin/lib/src/rvt/frontdoor/__init__.py` via sync), `tests/test_coldstart.py`, `docs/inbox/bare-ifc-prereq.md`.
...
- **Gates:** stream-local 47 passed; frontdoor + ifc-intent units 89 passed / 5 skipped; sync `--check` clean; validate_plugin PASS; portable paths ok; merged shard — SHARD_RESULT_PLACEHOLDER.
- **Open:** the two follow-ups in §6 (option A behind the #498 fence; bench BLOCKED classification in #113's file).
