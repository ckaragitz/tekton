...
| same, **`tools/route.py run --output rvt --prompt …`** | `T/samples/escape/r` | rc 0, ONE JSON `"ok": true, "status": "PROOF-ONLY (self-checks PASS…)"`, 12 files physically in `elsewhere-main/r` incl. `prompt_room.rvt` | rc 3, ONE JSON `"status": "FAILED (prompt->intent + intent->rvt: FrontDoorError: --out refused (nothing built): … than /home/user/tekton/samples/escape/r)"`; the engine step is refused and builds nothing — but the **router's own** `ROUTE.md` / `route.json` / `route.log` still land through the link before it calls the engine: that up-front check is #473 / PR #493's (router.py is outside this territory) and it inherits `_inside` the moment it calls `out_dir_refusal` |
...
* Branch `cam/474-out-dir-symlink` from `origin/main` @ e3579ea.
