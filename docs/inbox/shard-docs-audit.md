...
1. **`tests/test_gates_shared_walk.py`** — the private in-place `_rewrite_stream(src, dst, name, mutate)` (`mutate(bytearray)
   ...
   pure damage `_flip_bit(raw)` (the "one tiny local adapter" the DONE allows — kept local, not in conftest: it is this
   file's recipe at this file's `_IN_FIRST_MEMBER` offset, no second caller exists, and conftest was default-frozen for
   this issue), the `lambda raw: _smash64(raw, _LOST_BODY[name])` keeps its spelling (it now *returns* the damaged bytes),
...
`git diff --numstat`: `tests/test_gates_shared_walk.py` **+22 −45**, `tests/test_conftest_scaffolding.py` +5 −4. pyflakes
...
### /simplify pass — taken / not taken

SIMPLIFY_TBD

### Follow-ups

- None filed. The one thing found — this file passes under `no_release_leak`, so it can become an opt-in adopter — is a
  comment on the existing **#605** (its `ADOPTERS` / derived-adopters bullet), searched first; no new issue.
