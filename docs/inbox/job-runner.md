...
## eng #440 — 2026-08-10 — the `degradations` note is in the ON-DISK `.manifest.json` too

Stream: `eng440` (engineer session under the tech-lead session; branch `cam/440-degradations-in-manifest` from
`origin/main` @ 855f764; fresh cloud clone, no `samples/`). Closes #440; Refs #433 (wrote the patch out, could not land it
from its fence), #424 (introduced the note; its follow-up 3), #267 / #396 (the `--json` envelope). Territory used:
`tools/rvt_job.py` (`_RUN`, `_record_manifest`, `main()`, the module docstring's OUTPUT MODES sentence — nothing else),
`tests/test_stagelog.py` (the one squatted-log test), this section; mirrors regenerated by `tools/sync_plugin.py`
(`plugin/lib/tools/rvt_job.py`, `plugin/skills/tekton-{author,edit,native}/scripts/rvt_job.py`). Not touched:
`tools/frontdoor.py`, `src/rvt/**`, `tests/test_go_edit.py`, `tests/ci_shard.txt` / `tests/ci_shard.d/` (no new test file).

### What was built (the #433 patch, as written)

* `_RUN` gains a fourth run-state key `degradations` (reset to `[]` per `main()`, like the other three); `main()`'s
  `stage_stdout(…, on_degrade=_RUN["degradations"].append)` appends there instead of into a local list; `_record_manifest`
  — the one funnel every manifest of a run (full via `write_manifest`, stub via `_write_stub_manifest`) already passes
  through to get `output.log` named — copies a **non-empty** list into the manifest as top-level `degradations`. On the
  happy path the key is **absent, not `[]`**, so `tests/test_go_edit.py::test_go_ops_door_result_is_the_manifest`
  (printed == on-disk minus `exit_code`) stays byte-true untouched. `main()` no longer adds the key to the printed dict:
  the printed object is `dict(_RUN["manifest"], exit_code=rc)` and nothing else, so "printed JSON == manifest file +
  `exit_code`" is exact again on every path — happy, degraded, full manifest and stub alike.
* Timing is what makes the funnel sufficient: `rvt._logsink.stage_stdout` calls `on_degrade` at **open** time, before the
  `with` body (`_dispatch`) runs (`src/rvt/_logsink.py:58-64`), so the note is in `_RUN` before any manifest of the run
  is written — no manifest can miss it, and no second write of the file is needed.
* Module docstring OUTPUT MODES: the "printed object only (not to the on-disk manifest)" caveat (#433) is replaced by
  "adds ONE `degradations` note to the manifest (printed and on disk alike; the key is absent otherwise)".
* `tests/test_stagelog.py::test_rvt_job_json_with_a_squatted_log_still_delivers_one_json`: the comparison now excludes
  only `exit_code` (`{k: v … if k != "exit_code"} == on_disk`) and asserts `on_disk["degradations"] == doc["degradations"]`
  (the one `IsADirectoryError` note) — the file itself now says why `output.log` is absent.

### Evidence — bare surface, before / after

Bare = each build's `tekton-plugin.zip` (`tools/sync_plugin.py` from `origin/main` @ 855f764 = "before", from this head =
"after") unzipped under `/tmp/bare440/{before,after}`, system `/usr/local/bin/python3` 3.11.15 (no numpy), uid `nobody`
(`runuser -u nobody`), `env -u PYTHONPATH -u TEKTON_ROOT`, out dir with a space (`out dir/`), interleaved before/after ×3
per row. Flow: `skills/tekton-edit/scripts/_bootstrap.py go rvt_job.py edit <copy of assets/genesis/G_ABPD_2025.rvt>
--ops ops.json -o "out dir/e.rvt"` (`go` appends `--json`); `ops.json = {"ops":[{"op":"set-level","id":1351691,"elevation_ft":5}]}`;
"squat" = `out dir/e.rvt.log` pre-created as a **directory**.

| case | build | rc | stdout (ONE JSON) | stderr | printed `result` minus `exit_code` == on-disk manifest | `degradations` printed / on disk | `output.log` | delivered `e.rvt` | wall ×3 |
|---|---|---|---|---|---|---|---|---|---|
| happy | before | 0 | 10,571 B | **0 B** | **True** | absent / absent | named | 598,016 B | 1.06 / 0.96 / 0.96 s |
| happy | after | 0 | 10,562 B | **0 B** | **True** (no `degradations` key anywhere) | absent / absent | named | 598,016 B, **`cmp`-identical to before's** | 0.95 / 1.13 / 0.98 s |
| squat | before | 0 | 10,685 B | **0 B** | **False** — printed carries `degradations`, the file does not | 1 note / **absent** | absent | 598,016 B | 0.95 / 0.97 / 0.92 s |
| squat | after | 0 | 10,676 B | **0 B** | **True** | 1 note / **the same 1 note** (`e.rvt.log not writable (IsADirectoryError: [Errno 21] Is a directory: '…/out dir/e.rvt.log'); stage output not logged`) | absent | 598,016 B, `cmp`-identical to before's and to the happy output | 0.93 / 0.92 / 0.89 s |

Every row: `go.ready true`, `go.stdout` / `go.exception` absent, `result.status` `PROOF-ONLY, NOT-DELIVERABLE (hard gates
PASSED)`; `rvt_validate.py` on the after/squat `e.rvt` → `VALID (no errors); warnings=0` ("validates", never "loads" —
rule 4; no format byte changed). The 9-byte stdout differences are the build dir names inside echoed paths. Stub paths
(after build, same squatted dir): unplannable op (`id 999999999`) → rc 2, stderr = the one `[rvt_job] FAILED (planning:
…)` line, printed == on-disk + `exit_code` **True**, on-disk stub carries the note, `output.written false`; non-JSON
`--ops` (`_dispatch` path, `main()`'s own stub) → rc 1, one stderr line, printed == on-disk **True**, note on disk. Repo:
`tools/frontdoor.py author --rvt plugin/assets/genesis/G_ABPD_2025.rvt --edit "set level 311 elevation to 5 ft" --out
out/verify440/e --json` → rc 0, stderr 0 B, `PROOF-ONLY, NOT-DELIVERABLE (hard gates PASSED)`, `manifest` = `edit.log,
json, md`, `errors []`, `G_ABPD_2025.edited.rvt` → `VALID (no errors); warnings=0`, no `degradations` key in any file it
wrote (text-mode in-process job: `stage_stdout` is `--json`-only) — the `--rvt --edit` route is unchanged.

### Gates (final tree, after `/simplify` + `/verify`)

* `RVT_SKIP_LARGE=1 .venv/bin/python -m pytest tests/test_stagelog.py -q -rs` → **9 passed, 1 skipped** (the pre-existing
  `chmod 0555` case as root). With the neighbours `tests/test_go_edit.py tests/test_plugin_sync.py tests/test_bootstrap.py
  tests/test_coldstart.py tests/test_job.py tests/test_edit_own_release.py` → **59 passed, 5 skipped** (15 s; the 4
  `test_job.py` skips are `samples/` absent). `tests/test_go_edit.py::test_go_ops_door_result_is_the_manifest` untouched
  and green (the key is absent on the happy path).
* The whole merged CI shard (`RVT_SKIP_LARGE=1 .venv/bin/python -m pytest -q -p no:cacheprovider $(python3 tools/dev/shard_list.py --print)`): see the PR body for the counts of the run on this exact head.
* `tools/sync_plugin.py` (4 mirrors of `rvt_job.py` re-synced, deny-audit clean, identity scan == allowlist, zip 5,224 KB)
  then `--check` → "plugin in sync with source"; `plugin/scripts/validate_plugin.py` → PASS (25 assertions);
  `tools/dev/check_portable_paths.py` → ok (2906 paths). No new test file → no `tests/ci_shard.d/` drop-in.
* `/simplify` (4 lenses): applied — the `list(…)` copy in `_record_manifest` dropped (nothing appends after log-open, and
  `main()` rebinds a fresh list per run, so the copy only implied a hazard that does not exist); `on_degrade` made
  late-bound (`lambda note: _RUN["degradations"].append(note)`) so both callbacks write through `_RUN` symmetrically
  instead of one binding the list object created two lines earlier; the test asserts the note on the **on-disk** manifest
  and lets the printed == on-disk equality cover the printed side (no doubled assertions). Confirmed right as-is (reuse /
  altitude): `_record_manifest` is the one seam both writers share, the `{log, degradations}` + `on_open`/`on_degrade`
  shape is the convention `src/rvt/frontdoor/edit.py` / `build.py` already use, `_logsink` stays manifest-agnostic.
* `/verify` (final zip, bare unzip, uid nobody, system `python3` 3.11.15, no numpy): the table above re-run ×3 on the final
  head (happy: rc 0 / stderr 0 B / printed == on-disk **True**, no `degradations` anywhere / `e.rvt` `cmp`-identical to
  main's; squat: rc 0 / stderr 0 B / printed == on-disk **True**, the one note in both / `e.rvt` `cmp`-identical);
  `go author --prompt "an electrical room with 6 panels" --out "out j1"` from the same unzip → `go.ready true`, rc 0,
  stderr 0 B, 3.8 s, `PROOF-ONLY (self-checks PASS …)`, `manifest` = `build.log, json, md`, `files` = `combined,
  families_dir`; repo front door `--rvt … --edit` as in the evidence paragraph. Full suite NOT run (SUITE-COORDINATION).

### Follow-ups

None new. #433's unfiled observation 3 (the front door's `--rvt --edit --json` route carries the job's one abort line on
its own stderr) is unchanged by this PR and still belongs to the next holder of `src/rvt/frontdoor/edit.py`.

## BRANCH STATE (eng #440)

* Branch `cam/440-degradations-in-manifest` from `origin/main` @ 855f764; PR closes #440.
* Files written: `tools/rvt_job.py` (`_RUN["degradations"]` + its per-run reset, `_record_manifest` copies a non-empty
  list, `main()` appends there and prints the manifest + `exit_code` only, OUTPUT MODES sentence), `tests/test_stagelog.py`
  (the squatted-log test), `docs/inbox/job-runner.md` (this section). Regenerated mirrors (`tools/sync_plugin.py`):
  `plugin/lib/tools/rvt_job.py`, `plugin/skills/tekton-{author,edit,native}/scripts/rvt_job.py`.
* Not touched: `tools/frontdoor.py`, `src/rvt/**`, `tests/test_go_edit.py`, `plugin/skills/*/SKILL.md`,
  `tests/ci_shard.txt` / `tests/ci_shard.d/`, `TRACKER.md`, `KNOWLEDGE.md`, the ledger.
* Gates: as listed above, all green; nothing staged for the viewer (no format byte changed — both edit outputs are
  `cmp`-identical to `main`'s; no certification claim).
* Shipped vs staged: everything in this PR ships; nothing awaits a human.
* Scratch (not committed): `/tmp/bare440/{before,after,work}` (the two unzipped builds + every run's stdout/stderr),
  the scratchpad's `bare.sh` and `main-wt/` (an `origin/main` worktree used only to build the "before" zip).
