...
single-variable step with its own evidence, not a crash fix.  What *is* added is the
guard that turns that future mistake from silent into loud: `add_connector` now
refuses a host bundle that is not a straight 4-curve `prism_form` product
(`len(host.params["vertices"]) != 4` → `FactoryError`), because `box_face()` speaks
`_box_tags(4)` and would otherwise mis-tag a connector on a true cylinder (tag 1 is
that solid's *bottom* cap).  No current caller is affected (box, plate and
`polygon_cylinder` all carry 4 vertices); `test_connector_refuses_a_host_that_is_not_a_4_curve_prism`
pins it.
...
* `test_famgen_factory.py`: the six composition tests (already `needs_schema`) now
  ...
  `test_connector_refuses_a_host_that_is_not_a_4_curve_prism` pins the new guard.
...
|---|---|---|---|
| `test_famgen_factory.py` | 12 passed / 12 skipped | **24 passed / 5 skipped** (5 new) | 5 × rme/rst sample |
...
`tests/test_famgen_factory.py` and `tests/test_famgen_catalog.py` appended (+52
...
* **#298** (filed from this stream's `/simplify` review): one
  `rvt.schema.schema_available()` + a shared `needs_schema` in `tests/conftest.py`
  replacing the seven per-file probes (the four here, `test_residue_b.py`'s
  `_ensure_schema`, and the still-stale path probes in `test_genesis_skeleton.py:30`
  / `test_ifc_family.py:42`, which hide schema-only tests from CI the way #161's did).
* The true curved can (`G.cylinder` + a cylinder face map for `add_connector`) is a
  ...
  staged viewer batch.  Until then `add_connector` refuses such a host.
...
* Files written: `src/rvt/famgen/factory.py` (downlight can → `polygon_cylinder`;
  `add_connector` 4-curve-prism guard),
  ...
  new four-kinds test + guard test), `tests/test_famgen_skeleton.py`,
