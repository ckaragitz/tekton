...
* `plugin/scripts/validate_plugin.py` -> PASS (25 assertions); `tools/sync_plugin.py` + `--check` ->
  "plugin in sync with source"; `tools/dev/check_portable_paths.py` -> ok (3160 paths).
...
* `references/sop-harden-deliver.md` still walks the four-call runbook -- fine as the compose-by-hand
  detail; #112's territory.

## BRANCH STATE

* Branch `cam/757-skill-one-call` (from `main@385b1c3`). Files: `skills/tekton-ifc/SKILL.md` (source),
  `plugin/skills/tekton-ifc/SKILL.md` (mirror, via sync), this fragment + one index line.
