...
**Independent review of head `b8ae603` — 🛑, fixed on the branch.** The blocking finding was
real and new with this slice: a maker written next to a *not-built* kind was "outside every
clause", became the one loose maker and was stamped on every built item (`… and a 500 kW
Cummins generator` declared the panels and transformer Cummins; `… a 300 kW Generac generator`
put `Generac` into the house switchboard's Manufacturer parameter), and a Capitalised common
word outside any clause could do the same (`New York office` → YORK, `Price out …` → Price
Industries, `the Hammond plant` → the HPS record read for T1). Now (`_attach_makers`): every
maker goes to the **nearest equipment noun inside its clause, built or not** — a not-built kind
keeps its maker on its own record (`maker named for it: Cummins …`, an `understood` entry with
`applies_to`), two makers on one noun apply neither and say so, and a maker with no equipment
noun in its clause applies to the whole job **only on a cue** (`all gear by X`, `X equipment
throughout`; exactly one such maker); without the cue it applies to nothing — a warning for a
real maker's name, plain ignored words for `York`/`Price`. The nits: `fused disconnect switch`
aliases; `vendors._declared`'s memo keyed on `catalog.generation()` so `catalog.reload()` is
seen; the not-built line for a famspec-buildable kind names the request that builds it
(`route run --rfa '{"kind": "luminaire", "fixture": "recessed-troffer"}'`) instead of the room
...
- Files: `src/rvt/famgen/taxonomy.py`, `src/rvt/famgen/vendors.py`, `src/rvt/famgen/catalog.py`
  (`generation()`), `src/rvt/frontdoor/prompt_intent.py`, `src/rvt/ifc/intent.py`,
  `src/rvt/frontdoor/intent.py` (`summarize` carries plan degradations),
  `src/rvt/frontdoor/manifest.py`, `tools/make_family.py`, `tests/test_taxonomy_wiring_692.py` (new),
