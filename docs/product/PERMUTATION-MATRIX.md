...
- **Inputs** — any subset of `{prompt, ifc, rvt, rfa, spec}`
  ...
  luminaire | downlight, …}` — the written contract is
...
| rfa + rvt → rvt | **works** | `rfa_load` (load into **your** project: famspec (any of the four kinds) and standalone-born `.rfa` via `rvt.famload`; extracted `.rfa` via `rvt.famgen.loader`) | **certified** L1a / L_downlight (rst host), `stage_L8_lp4` (genesis lineage), **T2a** (Revit-born .rfa on the composed base + instance), **TB0g** (embedded-born famdoc on the composed base + instance); worked `DP1_reextracted.reloaded.rvt.load.json`; `tests/test_famload.py`, `tests/test_convert.py`, `tests/test_router.py` | input contract exactly as the rfa→rvt row (famspec per `spec/famspec.schema.json` \| tekton-extracted .rfa \| standalone-born .rfa via the id-remap lane); no instance is placed by this cell (place with prompt+rvt “add …”); on *your* host the same mechanisms + gates run — the viewer evidence is on the rst sample host and our genesis/composed bases; a host of another release loads only where that release’s creation support is certified; PROOF-ONLY |
...
- **downlight famspec kind on the bundled base** — `rvt.ifc.famfrom_ifc` should
  ...
  like the three catalog kinds already do (rfa → rfa / rfa → rvt, issue #162).
