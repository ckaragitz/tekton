...
| Board / planner / worker | `board.yml`, `techlead.yml`, `worker.yml` | the tech-lead loop, prompt `.github/prompts/tick.md`: an hourly server-side wake into the persistent tech-lead session **and** a one-shot *watchdog* routine that fires a fresh session only when no acting tick pushed its `run_once_at` forward (every acting tick sets it to now + `lease.minutes`, so it fires exactly that long after the last tick of a dead loop and never while one is alive), arbitrated by a LEASE — the body of issue `lease.issue` (#410) carries a plain `techlead-lease session=<session> until=<UTC>` line (not an HTML comment — MCP `issue_read` strips those from bodies — and no backticks: a shell would eat them); every fire — hold, take, standby or damaged alike — first re-arms the watchdog at the `ARM_WATCHDOG_AT` the tool prints (the lease in force + `lease.watchdog_margin_minutes`), then a fire that finds an unexpired lease held by another session stands by (exit 5), a damaged body is repaired not taken (exit 6), and otherwise it takes/renews for `lease.minutes` and acts (`techlead.py lease renew`, token-less via `--from-file <saved> --dry-run` + MCP `issue_write`; a watchdog-fired session releases at the end so the persistent one reclaims), so the loop survives its session without two loops acting in one tick — does the picture, the servicing, the fan-out to engineer sessions (≤ 5, disjoint territories — no file in common; each wave is written into the ledger the moment it starts: one comment on the board issue with `issue → engineer session id → territory` (`tools/dev/techlead.py wave post`, one `<!-- wave:… -->` marker per engineer), the sessions tagged `wave-<k>`; and every tick first rebuilds the live waves from that comment + its own session list (`techlead.py wave live`, also a line in `brief`; without `gh`/`GH_TOKEN` — the cloud tick — `issue_read` the board issue's comments and treat every `<!-- wave:<k> issue=<n> session=<id> territory=… -->` line that is not inside a `>` quote or a ``` fence as this loop's own engineer, latest line per (wave, issue) wins, so a take-over is re-posted under the same wave id; `wave live --from-file <saved comments JSON>` applies exactly those rules) before treating an unfamiliar engineer report as someone else's — a loop's memory can be compacted away, GitHub's cannot, #342) and the planning pass; the board issue is updated from that tick |
