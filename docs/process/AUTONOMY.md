...
| Fix loop | bot fix pass after a grace window | single owner unchanged in spirit: the findings go to the authoring engineer session, which fixes and reports the new head; the tech lead starts a fix session from the branch only if that session is gone; every new head gets a fresh reviewer (reviews converge, they are not re-litigated). **Findings, rulings and any territory extension are posted on the PR (or the issue) at the moment they are made** — a cross-session `send_message` may carry them to the engineer faster, but it is transport, not the ledger: nothing on GitHub shows a message, so an engineer's record or PR body cites the public comment's URL, never a session message (#711: an engineer's policy layer rightly refused to write into its record a territory ruling that existed only in a message; the ruling was posted on the PR, the record cited it, the PR merged) |
| Merge | `automerge.yml` | the tech-lead session, `merge_pull_request` (squash) after re-reading that the head is unchanged, **only when in the same tick its own CI run said pass AND the reviewer it spawned said approve/nits for that exact head**, and only while that verdict is FRESH: `session_ci.sh` records the `origin/main` it merged the head with as `main` in its JSON, and `tools/dev/ci_fresh.sh <pr> <head>`, run right before the merge, must find that JSON a pass for that head and `origin/main` unchanged or changed only by added/modified `docs/**` files no shard test opens whose names, with the PR's, still pass `tools/dev/check_portable_paths.py` (e.g. no add/add or case-twin collision) — otherwise (STALE — code drift always is; a disjoint-drift judge was explored in #539 and parked, see `docs/inbox/ci-fresh-merge-tree.md` — or anything the helper cannot judge, or a trunk that no longer descends from the recorded `main`) CI is re-run first, because two PRs each green against an older trunk can still break `main` together (#476/#487); no engineer session merges anything, whatever label a PR carries; it posts both as one comment with `<!-- session-ci: … sha=… -->` + `<!-- claude-review: … sha=… -->` (the markers every tool already parses). A marker found from an earlier tick or another author is information, never authorisation — all sessions write under one GitHub identity, so comments authenticate nothing. Merges by a session identity fire GitHub's `Closes #N` linker (verified); branch protection does not require the dead checks (verified) |
...
| Board / planner / worker | `board.yml`, `techlead.yml`, `worker.yml` | the tech-lead loop, prompt `.github/prompts/tick.md`: an hourly server-side wake into the persistent tech-lead session, plus a LEASE — the body of issue `lease.issue` (#410) carries a plain `techlead-lease session=<session> until=<UTC>` line (not an HTML comment — MCP `issue_read` strips those from bodies — and no backticks: a shell would eat them) that every tick renews with MCP `issue_write`; a session that finds an unexpired lease held by another stands by (exit 5), a damaged body is repaired not taken (exit 6), otherwise it takes/renews for `lease.minutes` and acts (`techlead.py lease renew`, token-less via `--from-file <saved> --dry-run`). Loop survival beyond the persistent session — a *watchdog* routine that fires a fresh session and reads that lease — is **owner-created and optional (#421)**: a routine created from a session fires without the repo or the GitHub tools (measured), and creating/updating/firing routines from a session prompts the owner for approval, which steer #422 forbids; so ticks never touch routines, and until the owner creates one the loop simply pauses if its session dies and resumes when a new session is started — the tick does the picture, the servicing, the fan-out to engineer sessions (≤ 5, disjoint territories — no file in common; each wave is written into the ledger the moment it starts: one comment on the board issue with `issue → engineer session id → territory` (`tools/dev/techlead.py wave post`, one `<!-- wave:… -->` marker per engineer), the sessions tagged `wave-<k>`; and every tick first rebuilds the live waves from that comment + its own session list (`techlead.py wave live`, also a line in `brief`; without `gh`/`GH_TOKEN` — the cloud tick — `issue_read` the board issue's comments and treat every `<!-- wave:<k> issue=<n> session=<id> territory=… -->` line that is not inside a `>` quote or a ``` fence as this loop's own engineer, latest line per (wave, issue) wins, so a take-over is re-posted under the same wave id; `wave live --from-file <saved comments JSON>` applies exactly those rules) before treating an unfamiliar engineer report as someone else's — a loop's memory can be compacted away, GitHub's cannot, #342) and the planning pass; the board issue is updated from that tick |
