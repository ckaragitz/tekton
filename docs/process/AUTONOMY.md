...
| Review | `claude-review.yml` | a fresh reviewer context spawned by the tech-lead session with `tools/dev/review_brief.md` (a subagent; for the tech lead's own PRs a separate reviewer session); it executes PR code only through the same sandbox; returns `✅/🟡/🛑` + `VERDICT=` |
...
What an **engineer session** does differently: claims by self-assigning; ignores every GitHub check
(they fail instantly with `runner_id 0` — meaningless); runs its gates locally and pastes counts;
