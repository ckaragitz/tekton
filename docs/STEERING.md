...
| S-2026-08-09-e | 2026-08-09 | ck | One owner per pipeline step. Where a live session and a bot could both act (fixing a red/🛑 PR), the session goes first: bots hold off pushing a fix for a grace window sized to CI + review latency, then a periodic sweep (not an immediate event trigger) takes over if the PR is still red. Nothing depends on the session staying alive beyond that window. | #67 / #72 |
