...
| 4 | one type per family; no labelled dimensions / formulas | the type table takes any number of rows; the loader loads them all (one host FamilySymbol + FamSymSurrogate per row is a small extension); label-driven geometry is the geometry stream's recipe §5 |
