...
| 8 | shared-parameter GUID identity — **BUILT, uncertified (#165, `docs/inbox/family-shared-params.md`)**: `make_* (shared_params=FILE)` / `make_family.py … --shared-params usecases/eaton-panelboard/panelboard-shared-parameters.txt` authors every parameter whose caption + datatype match a row of OUR shared-parameter file as a SHARED family parameter (`ParamElemExternal`, `revit.local.shared:<guid>` identity, `m_externalParamKey` = the file's GUID **verbatim** — the eleven contract params for the panelboard), everything else local; both loaders twin it keeping that identity; default (no flag) = every parameter local as before, now with **deterministic** local identities (`our_guid("family.parameters", family, caption)`, a uuid5, instead of a per-build uuid4); the policy is applied by `FamilyDoc.add_family_parameter` itself (`new_family_document(shared_params=)`), so every caller gets it. Policy: `skills/tekton-ifc/references/shared-parameters-mapping.md` §1.3. **Still open:** no corpus *family* carries a shared parameter, so the family-side definition + the host's `ExternalParamTracking` GUID registration on load are a certification-batch candidate (family-mode VALID 0 errors + provenance-clean today, not a loads claim) | until certified, route B (the project-side sidecar) remains the proven way to bind the firm's GUIDs |
