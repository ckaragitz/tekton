...
| Live sandbox reality (Cowork `pip install ifcopenshell`, wheel availability, egress) | in-progress | TRACKER F8 open. Local + macOS/manylinux wheels confirmed; a live Cowork tenant not yet tested. Fallback: pre-staged wheels (PLAYBOOK-cowork-and-chat). NEW: IFC *reads* (intent resolve, product facts) no longer depend on the install at all — the bundled stdlib-only steplite fallback engages automatically when ifcopenshell is absent (byte-identical outputs on both reference IFCs; `tests/test_steplite.py`) |
