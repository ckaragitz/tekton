...
(the ONE shared environment module).  Identical in every tekton skill.
    ...
    python scripts/_bootstrap.py go author --prompt "..." --out out/job1 --json
                                                        # THE ONE-CALL FLOW: inline preflight
                                                        #   + the job + ONE combined JSON --
                                                        #   a skill session needs exactly ONE
                                                        #   Bash call (`go SCRIPT.py ...` runs
                                                        #   any sibling script the same way)
    ...
    python scripts/_bootstrap.py run frontdoor.py author --prompt "..." --json
                                                        # two-call compat: run a sibling
                                                        #   script, engine ready (no pip, no
                                                        #   eval, no env prefix)
