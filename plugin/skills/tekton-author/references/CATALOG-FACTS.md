...
| duplex receptacle / switch / junction box (`receptacle_device`, prompt: “N duplex receptacles”, IFC: `IfcOutlet`) | `rvt.famgen.factory.make_device(kind, mounting_height_in=None, voltage=120, va=180)`; the structured request is the famspec kind `device` (`skills/tekton-native/examples/famspec.schema.json` + `famspec-device.json`, e.g. `{"kind": "device", "device": "duplex-receptacle" \| "duplex-receptacle-20a" \| "switch" \| "junction-box", "mounting_height_in": 18}`) through the router’s rfa cells — `rvt.frontdoor.router.route({"rfa": {…}}, "rfa" \| "rvt", out=DIR)` (repo: `tools/route.py run --rfa '{…}' --output rfa\|rvt`); a misspelt field or unknown device is ONE clear line | the `generic/devices-and-mounting` member: faceplate `plate` + device `box` at the record’s envelopes, ONE 1-pole 120 V primary connector (180 VA booked, bound to `Voltage` / `Apparent Load`), `Mounting Height` from the facts (18 in receptacles / 48 in switches unless given). **Honest status:** the famspec `--output rfa` delivers OUR `.rfa` family-mode VALID 0 errors + provenance clean, category Electrical Fixtures (−2001060) read back from the file, and `--output rvt` LOADS it unplaced onto the pinned base (four-registry, project validator 0 errors) — PROOF-ONLY; the room build lays the devices out at the ADA/NEC height, LOADS one shared family per distinct (kind, V, VA, height) with the Electrical Fixtures category and PLACES one instance per device (issue #359; manifest kind `fixture-instance` + a schedule row each; project validator 0 errors on 2026/2025/2024) — placed instances of our families on the composed base are the open cell, so the file is delivered STAMPED exactly like the boards; validator-green, not viewer-certified |
