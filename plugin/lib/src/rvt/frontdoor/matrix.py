...
extract->place (TB0g) depths cited on the rfa cells.
"""
...
_FOREIGN_SOURCE = ("a source/target that tekton did not author is DELIVERED "
                   ...
                   "dev-only stamp")
_RFA_INPUT = ("rfa INPUT CONTRACT: (a) a famspec JSON ({'kind': 'downlight', "
              "...}) -- the family is REBUILT by its constructor and loaded "
              "through the certified four-registry loader (rvt.famload); "
...
_RFA_HOST = ("default host = the pinned certified genesis base (G_ABPD, "
             ...
             "with instances")
_RFA_FAMSPEC_ENV = ("the famspec (downlight) lane emits its .rfa on the family "
                    "container archetype, which lives in the git-ignored "
                    "research corpus -- it runs on the owner machine only "
                    "until rvt.ifc.famfrom_ifc emits on the bundled base; the "
                    "extracted-.rfa lane runs anywhere")
...
_CELL_LIST: List[Cell] = [
    ...
    Cell(("rfa",), "rfa", STATUS_MISSING, None, (),
         ...
         missing_reason=("an .rfa alone with .rfa output is a no-op: family "
                         "MODIFICATION needs instructions -- prompt+rfa -> rfa "
                         "takes an edit sentence, inline JSON ops or an "
                         "ops.json path (rvt.convert.modify_family / "
                         "edit_family)"),
         closest=(("prompt", "rfa"), "rfa"),
         hint=("add the edit as the prompt: prompt+rfa -> rfa ('set BusRating "
               "225; rename the type to 225A MCB 42ckt'), or regenerate from "
               "changed facts: prompt->rfa")),
    # ---------------- singles: spec ----------------
