...
STAGES: Dict[str, Stage] = {s.id: s for s in [
    ...
    Stage("famspec->rfa", "rvt.frontdoor.famspec:build (rvt.famgen.factory:make_panelboard "
                          "| make_transformer | make_luminaire; rvt.ifc.famfrom_ifc:make_downlight)",
...
_RFA_INPUT = ("rfa INPUT CONTRACT: (a) a famspec JSON ({'kind': 'panelboard' | "
              "'transformer' | 'luminaire' | 'downlight', <the constructor's "
              "kwargs>} -- the written contract is spec/famspec.schema.json, "
...
_RFA_FAMSPEC_ENV = ("the catalog famspec kinds (panelboard / transformer / "
                    "luminaire) emit on the plugin-bundled base and run anywhere "
...
_CELL_LIST: List[Cell] = [
    ...
    Cell(("rfa",), "rvt", STATUS_WORKS, "rfa_load",
         ...
         ("certified:experiments/families/ifc/L_downlight_loaded.rvt",
          ...
          "record:docs/inbox/famspec-contract.md"),
         (_RFA_INPUT,
          "famspec lane (form a): all four kinds LOAD onto the pinned base with "
    ...
    Cell(("rfa",), "rfa", STATUS_WORKS, "rfa_generate",
         ...
         ("the rfa input here is a famspec JSON -- THE STRUCTURED REQUEST for one "
          "family: {'kind': 'panelboard' | 'transformer' | 'luminaire' | "
          "'downlight', <fields mirroring the constructor's keyword arguments>, "
          ...
          "assumed/user-given fact fields) -- measured on a fresh clone: panelboard "
          "45 / transformer 43 / luminaire 39 elements, VALID 0 errors, provenance ok",
