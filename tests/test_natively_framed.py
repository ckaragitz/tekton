...
def _foreign_pin() -> str:
    if not FOREIGN:
        pytest.skip("no certified foreign-release pin")
    return pinned_base(FOREIGN[0])
...
def test_foreign_pin_is_not_natively_framed(year):
    with open_rvt(pinned_base(year)) as doc:
...
@pytest.mark.parametrize("year", FOREIGN)
def test_predicate_follows_the_release_in_force(year):
    """Inside the file's own release context its headers parse: the predicate
...
@pytest.mark.parametrize("tool", TOOLS)
def test_shipped_mirror_is_the_source(tool):
    """The tekton-native script is a generated byte-identical mirror, so the
    law above holds for what ships too (sync_plugin --check owns drift; this
    is the cheap in-shard restatement)."""
    src = os.path.join(ROOT, "tools", f"{tool}.py")
    dst = os.path.join(ROOT, "plugin", "skills", "tekton-native", "scripts", f"{tool}.py")
    if not os.path.exists(dst):
        pytest.skip("plugin bundle not present")
