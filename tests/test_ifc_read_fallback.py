...
hold on a box that does have the wheel installed:
...
* ``tekton_env.ensure_engine`` composes with the engine helper (one entry).
"""
...
def _real_ifcopenshell_site() -> str | None:
    """The site dir holding a REAL ifcopenshell, if one is installed here."""
...
REAL_SITE = _real_ifcopenshell_site()
...
class _HideReal(importlib.abc.MetaPathFinder):
    ...
    everything else) alone: top-level ``ifcopenshell`` is looked up on
    sys.path minus the real install's site dir.  With no real install here a
    miss is an ordinary ``None`` (what an absent wheel yields); with one
    installed the miss must be FINAL (raise), or the default path finder
    behind us would serve the real library after all -- the engine helper
    treats a raising finder as 'absent' too."""

    def find_spec(self, name, path=None, target=None):
        if name != "ifcopenshell" or REAL_SITE is None:
            return None                     # submodules / nothing to hide
...
def no_real_ifcopenshell(monkeypatch):
    """In-process simulation of 'ifcopenshell is not installed'.  sys.path
    and sys.meta_path are swapped for copies (monkeypatch restores the
    originals); the ``ifcopenshell*`` module cache is purged and put back by
    hand, since modules imported DURING the test must go too.  Nothing is
    reloaded, so class identities elsewhere in the suite stay intact."""
    monkeypatch.setattr(sys, "path", [p for p in sys.path if p != SHIM])
    monkeypatch.setattr(sys, "meta_path", [_HideReal(), *sys.meta_path])
    monkeypatch.delenv(F.FORCE_ENV, raising=False)
    saved = {k: sys.modules.pop(k) for k in [k for k in sys.modules if _is_ifcos(k)]}
    try:
        yield
    finally:
...
def _fresh_intent_reader(monkeypatch):
    ...
    earlier test may already have bound them to the real library);
    monkeypatch restores the original bindings.  With the real wheel merely
    HIDDEN (still on disk) the shim's own stand-down probe would find it, so
    ``RVT_STEPLITE_FORCE`` pins the steplite backend -- what a genuinely
    absent wheel yields (proved for real in the record's no-ifcopenshell
    venv transcript)."""
...
def _run_py(code: str, *flags: str, marker: str) -> None:
    """``code`` in a fresh interpreter (repo cwd, no PYTHONPATH / plugin /
    FORCE env); must exit 0 and print ``marker``."""
    env = {k: v for k, v in os.environ.items()
           if k not in ("PYTHONPATH", "RVT_PLUGIN_ROOT", F.FORCE_ENV)}
    r = subprocess.run([sys.executable, *flags, "-c", code], cwd=ROOT, env=env,
                       capture_output=True, text=True, timeout=120)
    assert r.returncode == 0, r.stderr[-2000:]
    assert marker in r.stdout, r.stdout[-1000:]
...
def test_raising_finder_counts_as_absent(no_real_ifcopenshell, monkeypatch):
    ...
    assert F.ensure_ifc_reader() == SHIM          # still appends; the import then refuses
...
def test_package_import_selects_backend_in_a_fresh_interpreter():
    ...
    _run_py(
        ...
        "import ifcopenshell\n"
        "shim = bool(getattr(ifcopenshell, 'IS_STEPLITE', False))\n"
        "assert shim != had_real, (shim, had_real)\n"
        "assert (F.SHIM_DIR in sys.path) == shim\n"
...
def test_intent_from_ifc_resolves_example_via_steplite(no_real_ifcopenshell, monkeypatch):
    ...
    assert getattr(I.ifcopenshell, "IS_STEPLITE", False)   # proxy rebound to the shim
...
# ===========================================================================
# 3. the plugin bootstrap composes with the engine helper
# ===========================================================================
...
def test_tekton_env_ensure_engine_composes_with_engine_helper():
    ...
    if not os.path.isfile(os.path.join(PLUGIN, "lib", "src", "rvt", "ifc", "_fallback.py")):
    ...
    _run_py(
        ...
        "assert sys.path.count(F.SHIM_DIR) == 1 and sys.path[0] != F.SHIM_DIR, sys.path\n"
