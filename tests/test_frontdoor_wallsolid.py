...
* ``author --prompt "an electrical room 30 by 20 ft" --stages WV`` on each
  ...
  ``drawable_3d`` (6 faces, 12 edges), whose bBox is the wall's location
  line +- half the type thickness x 12 ft, equal to the header bBox;
...
* the mechanism is ``rvt.render.brep`` with the W1_gabpd_wall_solid recipe
  (root category -1, tags from the wall's own history);
* ``RVT_WALL_REP=dummy`` is a byte-exact opt-out: SerializedDummy reps, the
...
from rvt.render.inspect import inspect, _enter_release      # noqa: E402
...
def _build(out_dir: str, release: int, monkeypatch, wall_rep: str = None):
    ...
    kwargs = {"target_version": release} if release != 2026 else {}
    res = FD.author(prompt=PROMPT, out=out_dir, stages="WV", no_handoff=True, **kwargs)
    assert not res.errors, res.errors
    path = res.files.get("combined")
    assert path and os.path.isfile(path), res.files
    stages = (res.manifest.get("build") or {}).get("stages") or []
    wrec = [s for s in stages if s.get("stage") == "W"]
    assert wrec and wrec[0]["ok"], stages
...
def test_created_walls_carry_authored_solids(tmp_path, monkeypatch, release):
    ...
    # the release the file claims is the release asked for
    tv = res.manifest.get("target_version") or {}
    assert int(tv.get("output_release") or 2026) == release

    # stage W wrote the solid rep for every wall, by the certified recipe
    assert wrec["wall_rep"] == "solid"
    walls = wrec["walls"]
    assert len(walls) == 4
    for w in walls:
        rep = w["rep"]
        assert rep["kind"] == "solid" and rep["faces"] == 6 and rep["edges"] == 12
        assert rep["root_category_id"] == -1
        assert rep["tags"] == "history"                  # the wall's own BaseWallGStep tags
        assert abs(rep["height_ft"] - WALL_HEIGHT_FT) < EPS
        assert rep["thickness_ft"] > 0.1

    # read-back through the archaeology inspector (release-aware)
    rows = {r.element_id: r for r in inspect(path, classes=["SWall"])}
    assert set(rows) == {w["elem_id"] for w in walls}
    with contextlib.ExitStack() as stack:
        _enter_release(stack, path)
        ...
        for w in walls:
            ...
            hdr = doc.value(w["elem_id"], 101) or {}
            hbox = ((hdr.get("m_pBBox") or {}).get("value") or {}).get("m_minmax")
            assert hbox and _close(r.bbox, hbox), (r.bbox, hbox)
    ...
    assert lvr == MF.LOAD_VS_RENDER["solid"]
...
def test_dummy_opt_out_and_determinism(tmp_path, monkeypatch):
    ...
    res_d, path_d, wrec_d = _build(str(tmp_path / "dummy"), 2026, monkeypatch, wall_rep="dummy")
    ...
    _, a, _ = _build(str(tmp_path / "a"), 2026, monkeypatch)
    _, b, _ = _build(str(tmp_path / "b"), 2026, monkeypatch)
    assert _sha256(a) == _sha256(b)
    assert _sha256(a) != _sha256(path_d)
    assert os.path.getsize(a) >= os.path.getsize(path_d)
