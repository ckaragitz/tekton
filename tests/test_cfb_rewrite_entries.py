...
from rvt.roundtrip import catalog, read_entries, rewrite_entries, roundtrip
...
def _streams(path) -> dict:
    """``{stream path: raw bytes}`` of the container at ``path``, in directory order."""
...
def test_zero_full_trailers_only_keeps_every_logical_byte(pin, tmp_path):
    ...
    after = _streams(tmp_path / "v8.rvt")
...
def test_corrupt_trailer_bytes_flips_exactly_those_bytes(pin, tmp_path):
    ...
    after = _streams(tmp_path / "v9.rvt")
