...
def parse_prompt(prompt: str) -> ParsedPrompt:
    ...
    low = text.lower()
    consumed = np.zeros(len(text), dtype=bool)
    cov = PromptCoverage()
    ...
    cov.consumed_ratio = float(consumed.sum()) / max(1, len(text))
...
def _upright_frame(front: Sequence[float]) -> List[List[float]]:
    fz = np.array([float(front[0]), float(front[1]), 0.0])
    n = float(np.linalg.norm(fz)) or 1.0
    fz = fz / n
...
def prompt_to_intent(prompt: str) -> Tuple[I.IntentModel, ParsedPrompt]:
    ...
    for k, it in enumerate(parsed.buildable_items):
        ...
        plc = I.Placement(np.eye(4), [], ["prompt-authored: no IFC placement chain"])
