...
straight through as `--target-version {2026,2025}` — the tool owns the
...
- **Older releases** — same honest degrade: deliver what can be built today
  and the IFC addition; a version-N `.rvt` needs a version-N certified base.
...
Useful flags (see `--help`): `--target-version {2026,2025}` (the user's
answer from step 0 — the tool checks it and degrades honestly, see above),
...
2. `status` verbatim. `DELIVERABLE (all gates passed; genesis base)` is the
   ...
   (a 2025 target while the 2025 base certifies), relay that line verbatim
   with the files — the `.rvt` AND the IFC addition are both the user's.
...
1. **Build inputs.** All build inputs ship in the plugin: the family
   ...
   certification (step 0) — where `$RVT_FAMILY_DONOR` / `--specimens` /
   `--base` remain as expert overrides.
