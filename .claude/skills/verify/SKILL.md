...
| router / matrix (`src/rvt/frontdoor/{router,matrix}.py`, `tools/route.py`) | `$PY tools/route.py matrix` · `$PY tools/route.py explain --in prompt --out rvt` · `$PY tools/route.py run …` | the honest table; no cell claims "works" without evidence (`verify_evidence()` backs it) |
